-- Purpose: Build Dataset for Progression through Dribbling %
-- Source: FBref team tables (uploaded to BigQuery as CSV)
  
SELECT
  League,
  Squad,
  Poss,
  LeaguePosition,
  ROUND(Prg_P/MP,3) as prg_P_90,
  ROUND(npxG_Sh,4) as npxg_per_shot,
  ROUND(Prg_Carries/MP,3) as prg_C_90,
  ROUND((Prg_P/MP)/((Prg_P/MP)+(Prg_Carries/MP)),3) AS pct_prg_P_90,
  ROUND((Prg_Carries/MP)/((Prg_P/MP)+(Prg_Carries/MP)),3) AS pct_prg_C_90,
  ROUND((Prg_P/MP)+(Prg_Carries/MP),3) AS total_prg_90
FROM `football-half-season-analysis.top_5_football.master`;
